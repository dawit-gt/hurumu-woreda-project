...
export default function UsersAdminPage() {
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      await api.patch(`/users/${selectedId}`, form);
  ...
  const handleDeactivate = async (user: User) => {
    ...
    try {
      ...
      await mutate();
      if (selectedId === user.id) resetForm();
    } catch (err: any) {
      setMessage(
      ...
      <div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500">
            Manage the user accounts in your system.
          </p>
        </div>
        <button
          type="button"
        ...
        <section className="rounded-3xl border border-gray-200 bg-white p-6 shadow-sm">
          <div className="mb-4 flex items-center justify-between gap-4">
            <div>
              ...
              <p className="text-sm text-gray-500">
                Admin-only actions are available to super admins.
              </p>
            </div>
            ...
            <form className="space-y-4" onSubmit={handleSubmit}>
              ...
              {!editable ? (
                <div className="rounded-2xl bg-yellow-50 px-4 py-3 text-sm text-yellow-700">
                  Only super admins may update or deactivate users.
                </div>
              ) : null}
            </form>
          ) : (
            <div className="rounded-3xl bg-gray-50 p-6 text-sm text-gray-600">
